...
namespace unvell.ReoGrid.Demo.CellTypeDemo
{
	...
	class LoadingCell : CellBody
	{
		...
		public void NextFrame()
		{
			...
			{
				offset += StepSize;
				if (offset >= this.Cell.GetBounds().Width - ThumbSize - StepSize) dir = -1;
			}
			else
			{
				offset -= StepSize;
		...
		{
			// 長方形を描画
			dc.Graphics.FillRectangle(new Rectangle(offset, 0, ThumbSize, this.Cell.GetBounds().Height), SolidColor.SkyBlue);
	...
	class GifImageCell : CellBody
	{
		...
		public override void OnPaint(CellDrawingContext dc)
		{
			lock (this.Gif) dc.Graphics.DrawImage(Gif, this.GetBodyBounds());
	...
	class BlinkCell : CellBody
	{
		...
		public override void OnPaint(CellDrawingContext dc)
		{
			dc.Graphics.FillRectangle(this.GetBodyBounds(), new SolidColor(alpha, SolidColor.Orange));
