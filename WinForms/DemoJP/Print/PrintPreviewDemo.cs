...
namespace unvell.ReoGrid.Demo.PrintDemo
{
	public partial class PrintPreviewDemo : UserControl
	{
		...
		protected override void OnLoad(EventArgs e)
		{
			...
			this.worksheet.Scaled += (_, _) =>
			{
				this.tckZoom.Value = (int)(this.worksheet.ScaleFactor * 100);
				labZoom.Text = this.tckZoom.Value + "%";
			};
		}
		...
		{
			this.worksheet.ScaleFactor = (float)Math.Round(tckZoom.Value / 10f) / 10f;
			labZoom.Text = Math.Round(this.worksheet.ScaleFactor * 100) + "%";
		}
	}
}
